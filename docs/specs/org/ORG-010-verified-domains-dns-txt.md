...
traceability:
  code:
    ...
    - packages/registry-core/src/org-ports.ts
    - packages/db/src/adapters/d1-org-domains.ts
    ...
    - apps/web/src/routes/api.orgs.$org.domains.ts
    - apps/web/src/components/org/domains-card.tsx
  tests:
    - packages/registry-core/src/org.test.ts
    - packages/db/src/adapters/d1-org-domains.test.ts
    - packages/db/src/adapters/hmac-domain-challenge.test.ts
---

## Description

An org admin can prove control of a domain and earn a public "verified" badge. The
challenge is **stateless**: the expected TXT value is `HMAC(server-secret, "<org>:<domain>")`,
derived on demand (nothing per-domain is stored), published by the org at
...
keeps the verified set honest as DNS changes.
...
Then "acme.dev" is marked verified and shows a badge on "/orgs/acme"
```
...
And verifying a domain the org has not claimed is not found
```

### ORG-010-AC3 , scheduled re-verification revokes stale domains, skips DNS errors
```gherkin
Given organisation "acme" has a verified domain "acme.dev"
