...
`user`, `session`, `account`, and `verification` tables are stored in D1. A session cookie
references a server-side `session` row (created at sign-in, deleted at sign-out, expiring at
its stored expiry) rather than the prior stateless `<userId>.<hmac>` token (`AUTH-003`). This
supersedes the stateless session and absorbs who-am-i (`AUTH-005`) and sign-out (`AUTH-006`)
into BetterAuth's session endpoints.
