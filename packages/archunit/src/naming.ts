...
/** PascalCase identifier: an uppercase letter then letters/digits (e.g. `D1OrgStore`). */
...
/** kebab-case token: lowercase words joined by single hyphens (e.g. `d1-org-store`). */
export const kebabCase = /^[a-z0-9]+(-[a-z0-9]+)*$/;

/**
 * A pattern for a kebab-case filename with one of `extensions` (default `ts`/`tsx`):
 *
 *   kebabFilename()            //  d1-org-store.ts | route.tsx
