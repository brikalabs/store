...
function OperatorOrgsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (res.ok) setOrgs(((await res.json()) as { orgs: OperatorOrg[] }).orgs);
  }, []);
  useEffect(() => {
    void load();
  }, [load]);
  ...
  const act = useCallback(
    async (slug: string, path: string, body?: unknown) => {
      ...
      if (res.ok) {
        ...
        setError(((await res.json()) as { error?: string }).error ?? "Action failed");
      }
    },
    [load],
  );
  ...
  return (
    <OperatorShell activeLabel="Organisations">
      ...
      {orgs === null ? (
        ...
        <p className="text-muted-foreground text-sm">No organisations match.</p>
      ) : (
