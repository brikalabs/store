...
export function LogoCard({ org, onError }: Readonly<OrgCardProps>) {
  ...
  async function upload(event: ChangeEvent<HTMLInputElement>) {
    ...
    setBusy(true);
    const res = await fetch(orgPath(org, "/icon"), {
      method: "POST",
  ...
  async function clear() {
    setBusy(true);
    const res = await fetch(orgPath(org, "/icon"), { method: "DELETE" });
    setBusy(false);
    if (res.ok) setBust((n) => n + 1);
    else onError(await readError(res));
  }
  ...
  return (
    <section className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-6">
      ...
      <p className="text-muted-foreground text-sm">
        A PNG, JPEG, or WebP up to 512 KiB, shown on your public org page. Without one, a generated
        avatar is used.
      </p>
      <div className="flex items-center gap-4">
        <img
          // The cache-buster forces a reload after upload/clear; a 404 falls back to the gradient.
          src={`${orgPath(org, "/icon")}?v=${bust}`}
          alt="Current logo"
