...
interface MembersCardProps extends OrgCardProps {
...
/** The org's members + roles (admins manage; the last-admin invariant is enforced server-side). */
export function MembersCard({
  ...
  async function remove(member: Member) {
    const res = await fetch(orgPath(org, `/members/${encodeURIComponent(member.id)}`), {
      method: "DELETE",
    });
    if (res.ok) await onReload();
    else onError(await readError(res));
  ...
  return (
    <section className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-6">
      ...
      {isAdmin && <AddMember org={org} onAdded={onReload} onError={onError} />}
    </section>
  );
}
...
function AddMember({
  ...
  onAdded,
  onError,
}: Readonly<OrgCardProps & { onAdded: () => Promise<void> }>) {
